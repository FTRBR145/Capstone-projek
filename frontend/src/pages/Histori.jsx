...
function buildRunningBalance(transactions) {
  ...
  for (const t of sorted) { bal += t.type === 'pemasukan' ? t.amount : -t.amount; map.set(t.id, bal) }
  return map
}
...
export default function Histori() {
  ...
  const handleAmountChange = (e) => {
    ...
    setAmountDisplay(f); setFormData(p => ({ ...p, amount: parseNum(f).toString() }))
  }
    ...
    try {
      ...
      if (editItem) {
        ...
        if (formData.wishlistId) {
          try {
            const w = wishlists.find(x => x.id === parseInt(formData.wishlistId, 10))
            if (w) {
              ...
              toast.success(`Tabungan "${w.name}": Rp ${formatRupiah(w.saved_amount)} → Rp ${formatRupiah(ns)}`)
            }
  ...
  const handleDelete = async (id) => {
    ...
    setLoading(true)
    try { await transactionService.deleteTransaction(id); toast.success('Transaksi berhasil dihapus'); await fetchAllData() }
    ...
    <div className="animate-fade-in text-white pb-10">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-2.5 mb-5">
        ...
        <div className="rounded-xl p-3 min-[901px]:p-3.5 flex flex-col gap-1.5 bg-gradient-to-br from-sky-700/90 to-indigo-900 ring-1 ring-white/[0.08] shadow-[0_4px_16px_rgba(59,130,246,0.10)]">
          ...
          <p className="text-[17px] font-semibold leading-tight tabular-nums">Rp {formatRupiah(summary.pemasukan)}</p>
        </div>
        <div className="rounded-xl p-3 min-[901px]:p-3.5 flex flex-col gap-1.5 bg-gradient-to-br from-rose-800/80 to-slate-900 ring-1 ring-white/[0.08] shadow-[0_4px_16px_rgba(239,68,68,0.09)]">
          ...
          <p className="text-[17px] font-semibold leading-tight tabular-nums">Rp {formatRupiah(summary.pengeluaran)}</p>
        </div>
      </div>
            ...
            : paginatedRows.map(item => (
              <div key={item.id} className="tq-card-inner p-3.5">
                ...
                <div className="grid grid-cols-2 gap-y-1 text-[11px] mb-2.5">
                  ...
                  <span className={`text-right font-mono ${item.type === 'pemasukan' ? 'text-emerald-400' : 'text-red-400'}`}>
                    {item.type === 'pemasukan' ? '+' : '-'} Rp {formatRupiah(item.amount)}
                  </span>
                </div>
                <div className="flex justify-end gap-4 border-t border-white/[0.07] pt-2">
        ...
        <div className="hidden md:block overflow-x-auto rounded-xl border border-white/[0.07]">
          <table className="w-full min-w-[780px]">
            <thead className="bg-white/[0.04]">
              <tr>
                {['Tanggal', 'Keterangan', 'Jenis', 'Pengeluaran', 'Pemasukan', 'Saldo Berjalan', 'Aksi'].map(h => (
                  <th key={h} className={`px-3 py-2.5 text-[11px] font-semibold text-white/40 uppercase tracking-wider ${['Pengeluaran','Pemasukan','Saldo Berjalan'].includes(h) ? 'text-right' : h === 'Aksi' ? 'text-center' : 'text-left'}`}>
                    {h}
                  </th>
                ))}
              </tr>
            ...
            <tbody className="divide-y divide-white/[0.05]">
              {paginatedRows.length === 0
                ? <tr><td colSpan={7} className="py-10 text-center text-[13px] text-white/35">Tidak ada transaksi</td></tr>
                : paginatedRows.map(item => (
                  <tr key={item.id} className="hover:bg-white/[0.03] transition-colors">
                    ...
                    <td className="px-3 py-3">
                      <span className={`tq-badge ${item.type === 'pemasukan' ? 'tq-badge-income' : 'tq-badge-expense'}`}>
                    ...
                    <td className="px-3 py-3 text-center whitespace-nowrap">
                      <button onClick={() => openEditModal(item)} className="text-[12px] text-blue-400 hover:text-blue-300 mr-3 transition">Edit</button>
                      <button onClick={() => handleDelete(item.id)} className="text-[12px] text-red-400 hover:text-red-300 transition">Hapus</button>
                    </td>
                  </tr>
                ))
              }
      ...
      {/* Modal */}
      <Modal isOpen={modalOpen} onClose={() => setModalOpen(false)} title={`${editItem ? 'Edit' : 'Tambah'} ${modalType === 'pemasukan' ? 'Pemasukan' : 'Pengeluaran'}`}>
        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            <label className="block text-[12px] text-white/50 mb-1.5">Tanggal</label>
            <input type="date" value={formData.date} onChange={e => setFormData(p => ({ ...p, date: e.target.value }))} className="tq-field w-full px-3 py-2.5 text-[13px]" required />
          </div>
          <div>
            ...
            <div className="relative">
              ...
              <input type="text" inputMode="numeric" value={amountDisplay} onChange={handleAmountChange} placeholder="0" className="tq-field w-full pl-8 pr-3 py-2.5 text-[13px]" required />
            </div>
          </div>
          {!editItem && wishlists.length > 0 && (
            <div>
              ...
              <select value={formData.wishlistId} onChange={e => setFormData(p => ({ ...p, wishlistId: e.target.value }))} className="tq-field tq-select w-full px-3 py-2.5 text-[13px]">
                <option value="">— Pilih Wishlist —</option>
                {wishlists.map(w => <option key={w.id} value={w.id}>{w.name} ({formatRupiah(w.saved_amount)}/{formatRupiah(w.target_amount)})</option>)}
              </select>
            </div>
          )}
          <div>
            <label className="block text-[12px] text-white/50 mb-1.5">Keterangan</label>
            <textarea value={formData.description} onChange={e => setFormData(p => ({ ...p, description: e.target.value }))} className="tq-field w-full px-3 py-2.5 text-[13px] resize-none" rows={3} required />
          </div>
          <div className="flex gap-2 pt-1">
            <button type="button" onClick={() => setModalOpen(false)} className="flex-1 rounded-xl border border-white/10 bg-white/[0.06] py-2.5 text-[13px] font-medium text-white transition hover:bg-white/[0.1]">Batal</button>
            <button type="submit" disabled={loading} className={`flex-1 rounded-xl py-2.5 text-[13px] font-semibold text-white transition hover:brightness-110 disabled:opacity-60 ${modalType === 'pemasukan' ? 'bg-gradient-to-r from-emerald-600 to-teal-600' : 'bg-gradient-to-r from-rose-600 to-red-700'}`}>
