...
export default function Dashboard() {
  ...
  const handleQuickSubmit = async (e) => {
    ...
    if (modalType === 'pengeluaran' && amount > summary.saldo) { toast.error('Saldo tidak cukup!'); return }
    ...
    try {
      ...
      toast.success(`${modalType === 'pemasukan' ? 'Pemasukan' : 'Pengeluaran'} berhasil ditambahkan`)
      setModalOpen(false)
  ...
  return (
    <div className="text-white pb-10 animate-fade-in">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-2.5 mb-5">
        ...
        {/* Wishlist – muted violet/slate */}
        <div className="relative rounded-xl p-3 min-[901px]:p-3.5 flex flex-col gap-2 overflow-hidden shadow-[0_4px_16px_rgba(99,102,241,0.08)] ring-1 ring-white/[0.08] bg-gradient-to-br from-slate-700 to-slate-900">
          <div className="flex justify-between items-center">
            <span className="text-slate-300/70 text-[10px] font-medium uppercase tracking-widest">Wishlist</span>
            <span className="text-slate-300/35"><svg className="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path d="M20.84 4.61a5.5 5.5 0 0 0-7.78 0L12 5.67l-1.06-1.06a5.5 5.5 0 0 0-7.78 7.78l1.06 1.06L12 21.23l7.78-7.78 1.06-1.06a5.5 5.5 0 0 0 0-7.78z" strokeWidth={2}/></svg></span>
          </div>
          <div className="text-white text-[18px] font-semibold leading-none">{avgProgress}<span className="text-[13px] font-normal text-white/50">%</span></div>
          <p className="text-slate-400/80 text-[10px] m-0">{wishlists.length} aktif</p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-[1fr_300px] gap-5">
        ...
        <div className="tq-card relative p-5">
              ...
              {[
                ...
                <button
                  ...
                  className={`px-4 py-1.5 text-[13px] font-medium transition ${
                    activeChart === tab.key
                      ? tab.key === 'pemasukan' ? 'bg-emerald-600/30 text-emerald-300' : 'bg-rose-600/30 text-rose-300'
          ...
          <ResponsiveContainer width="100%" height={230}>
            <BarChart
              data={activeChart === 'pemasukan' ? chartData.pemasukan : chartData.pengeluaran}
            ...
            >
              ...
              <Bar
                dataKey="amount"
                fill={activeChart === 'pemasukan' ? '#10b981' : '#ef4444'}
                radius={[4, 4, 0, 0]}
                barSize={32}
              />
            </BarChart>
      ...
      {/* Quick Add Modal */}
      <Modal isOpen={modalOpen} onClose={() => setModalOpen(false)} title={`Tambah ${modalType === 'pemasukan' ? 'Pemasukan' : 'Pengeluaran'}`}>
